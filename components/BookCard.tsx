...
export default function BookCard({ book, onDelete, onEdit }: BookCardProps) {
  ...
  return (
    <article className={`book-card${isRead ? ' book-read' : ' book-unread'}`}>
      {/* Status badge */}
      <div className={`read-badge ${isRead ? 'badge-read' : 'badge-unread'}`}>
        {isRead ? (
          <>
      ...
      <div className="book-cover-wrap">
        ...
        <div className="card-actions">
          <button className="card-action-btn edit-btn" onClick={() => onEdit(book)} title="Editar livro" aria-label={`Editar ${book.title}`}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
            </svg>
          </button>
          <button className="card-action-btn delete-btn" onClick={() => onDelete(book)} title="Remover livro" aria-label={`Remover ${book.title}`}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      <div className="book-info">
        ...
        {book.comment && (
          <div className="card-comment-wrap">
            ...
            {showComment && (
              <p className="card-comment-text">{book.comment}</p>
            )}
          </div>
        )}
      </div>
